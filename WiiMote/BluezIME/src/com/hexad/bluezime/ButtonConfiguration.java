...
public class ButtonConfiguration extends PreferenceActivity {
    ...
    @Override
    /**
     * This constructor creates all the preferences and lists for button choices.
     * None of which are actually shown to the user.
     * 
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        m_presetName = (EditTextPreference) this
                .findPreference("rename_preset");
        ...
        for (Field f : KeyEvent.class.getDeclaredFields()) {
            ...
            if (name.startsWith("KEYCODE_"))
                try {
                    m_name_lookup.put(f.getInt(null),
                            name.substring("KEYCODE_".length()));
                } catch (Exception e) {
                }
        }
        ...
        PreferenceCategory buttonCategory = (PreferenceCategory) this
                .findPreference("cat_buttons");
        Preference resetButton = (Preference) this
                .findPreference("reset_button");
        resetButton
    ...
    private void updateDisplay() {
        for (Preference p : Collections.list(m_list_lookup.keys())) {
            ...
            lp.setSummary(m_name_lookup.containsKey(code) ? m_name_lookup
                    .get(code) : "UNKNOWN - 0x" + Integer.toHexString(code));
        ...
        for (int i = 0; i < profileEntries.length; i++) {
            ...
            profileEntries[i] = m_prefs
                    .getProfileDisplayName(Preferences.PROFILE_KEYS[i]);
        }
        ...
        if (currentProfile.endsWith(":"))
            currentProfile = currentProfile.substring(0,
