...
/**
 * This class takes care of all the settings and preferences and such for the service and adapter.
 */
public class BluezIMESettings extends PreferenceActivity {
    ...
    //TODO: Done by the original author for hopefully a good reason.
    @SuppressWarnings("unused")
    private Object m_donationObserver;

    //TODO: Also done by the original author for a good reason unclear to me.
    @SuppressWarnings("rawtypes")
    /**
     * This method instantiates all the fields and preference objects. It does most everything
     * else as well.
     * 
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        m_donateButton
                .setEntries(new CharSequence[] { getString(R.string.preference_use_paypal) });
        m_donateButton.setEntryValues(new CharSequence[] { "PAYPAL" });

        try {
            // This code enables the in-app donation system, but does not
            // require it for compilation
            // This is done to avoid polluting the project source with all the
            // boilerplate code
            ...
            Class c = loader
                    .loadClass("com.hexad.bluezime.donation.DonationObserver");
 ...
 * 
 * @Override public boolean onPreferenceClick(Preference preference) { try {
 * String url = "http://code.google.com/p/android-bluez-ime/"; Intent browse =
 * new Intent( Intent.ACTION_VIEW , Uri.parse( url ) ); startActivity( browse );
 * } catch (Exception e) { }
 * 
 * return false; } });
 */
        ...
        if (blue == null) {
            ...
            // m_configureButton.setIntent(new Intent(this,
                    ...
                    .setOnPreferenceClickListener(new OnPreferenceClickListener() {
                        public boolean onPreferenceClick(Preference preference) {
                            ...
                            if (BluetoothAdapter.getDefaultAdapter()
                                ...
                                dlg.setNegativeButton(R.string.no,
                                        new DialogInterface.OnClickListener() {
                                            @Override
                                            public void onClick(
                                                    DialogInterface dialog,
            ...
            registerReceiver(bluetoothStateMonitor, new IntentFilter(
            ...
            m_pairedDevices
                    .setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
                        ...
                        public boolean onPreferenceChange(
                            ...
                            if (newValue != null
                                    && newValue.equals(SCAN_MARKER)) {
                                startActivityForResult(new Intent(
                                        BluezIMESettings.this,
                                        DeviceScanActivity.class),
                                        DISCOVER_DEVICE_COMPLETE);
                                return false;
                            } else {
                                String address = (String) newValue;
                                m_prefs.setSelectedDevice(
                                        m_pairedDeviceLookup.get(address),
                                        address);
                                return true;
                            }
            ...
            // TODO do I have to do something here to default to Wiimote driver?
 ...
 * newValue) { String driver = (String)newValue;
 * m_prefs.setSelectedDriverName(driver); return true; } });
 */
            m_prefs.setSelectedDriverName(WiimoteReader.DRIVER_NAME);
        ...
        String[] displayNames = this.getResources().getStringArray(
                R.array.driver_displaynames);
        ...
        registerReceiver(preferenceUpdateMonitor, new IntentFilter(
        ...
        m_selectIME
                .setOnPreferenceClickListener(new OnPreferenceClickListener() {
                    @Override
                    public boolean onPreferenceClick(Preference preference) {
                        InputMethodManager m = (InputMethodManager) BluezIMESettings.this
    ...
    @Override
    /**
     * This method stops the class and unregisters some of the receivers.
     */
    ...
    @Override
    /**
     * Sets some values depending on what it finds.
     * 
     * @param requestCode The activity ID
     * @param resultCode Whether the activity succeeded or not
     * @param data Any other information the activity returned
     */
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);

        if (requestCode == DISCOVER_DEVICE_COMPLETE
                && resultCode == Activity.RESULT_OK) {
            BluetoothDevice device = (BluetoothDevice) data
                    .getParcelableExtra(DeviceScanActivity.EXTRA_DEVICE);
    ...
    /**
     * This method updates whether or not the user donated?
     * 
     * @param itemid 
     */
    public void updateDonationState(String itemid) {
        ...
        try {
            int purchased = Integer.parseInt(itemid.substring(itemid
                    .indexOf("_") + 1));
    ...
    /**
     * This method is how it determines whether a user donated?
     * 
    ...
    public void inAppDonationsEnabled(boolean enabled) {
        if (m_prefs.getDonatedAmount() > 0) {
            m_donateButton
                    .setTitle(R.string.preferencelist_donate_short_donated);
            m_donateButton
                    .setSummary(String.format(
                            this.getString(R.string.preferencelist_donate_long_donated),
                            m_prefs.getDonatedAmount()));
        } else {
    ...
    /**
     * Lists the devices that are paired to the phone.
     */
    private void enumerateBondedDevices() {
        ...
        CharSequence[] entries = new CharSequence[pairedDevices.size() + 1
                + ((containsCurrent || curDevice == null) ? 0 : 1)];
        CharSequence[] entryValues = new CharSequence[entries.length];

        if (pairedDevices.size() > 0) {
            BluetoothDevice[] devices = new BluetoothDevice[pairedDevices
                    .size()];
            ...
            for (int i = 0; i < devices.length; i++) {
                ...
                m_pairedDeviceLookup.put(devices[i].getAddress(),
                        devices[i].getName());
            }
        }

        /*
 * if (!containsCurrent && curDevice != null) { entries[entries.length - 2] =
 * m_prefs.getSelectedDeviceName(); entryValues[entries.length - 2] =
 * m_prefs.getSelectedDeviceAddress(); }
 */

        entries[entries.length - 1] = this
                .getString(R.string.bluetooth_scan_menu);
    ...
    /**
     * This method updates the display with any new information.
     */
    private void updateDisplay() {
        ...
        if (m_prefs.getSelectedDeviceAddress() == null) {
            ...
            m_pairedDevices.setSummary(m_prefs.getSelectedDeviceName() + " - "
                    + address);
        ...
        // m_configureButton.setEnabled(m_prefs.getSelectedDriverName() != null
        // && m_prefs.getSelectedDriverName().length() > 0);
    }
    ...
    private BroadcastReceiver bluetoothStateMonitor = new BroadcastReceiver() {
        ...
        @Override
        /**
         * This method returns the state of bluetooth on the phone.
         */
        public void onReceive(Context context, Intent intent) {
            ...
            if (state == BluetoothAdapter.STATE_ON) {
                ...
                m_bluetoothActivity.setEnabled(false);
                m_bluetoothActivity
                        .setSummary(R.string.bluetooth_state_turning_off);
            } else if (state == BluetoothAdapter.STATE_TURNING_ON) {
                m_bluetoothActivity.setChecked(false);
                m_bluetoothActivity.setEnabled(false);
                m_bluetoothActivity
                        .setSummary(R.string.bluetooth_state_turning_on);
            }
        }
    };
    ...
    private BroadcastReceiver preferenceUpdateMonitor = new BroadcastReceiver() {
        @Override
        /**
         * This method just updates the display.
         */
        public void onReceive(Context context, Intent intent) {
            updateDisplay();
        }
